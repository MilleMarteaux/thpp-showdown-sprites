...
calculateModifiedStats=function calculateModifiedStats(clientPokemon,serverPokemon){var _clientPokemon$volati;
...
if(item==='powerribbon'&&!(clientPokemon!=null&&clientPokemon.volatiles['dynamax'])){
stats.atk=Math.floor(stats.atk*1.5);
}
...
if(weather){
if(this.battle.gen>=3&&this.pokemonHasType(pokemon,'Rock')&&weather==='sandstorm'){
stats.spd=Math.floor(stats.spd*1.5);
}
...
if(item==='kusanagi'&&species==='Rinnosuke'){
stats.spa*=2;
}
...
if(item==='yatamirror'&&species==='Rinnosuke'){
stats.spd*=2;
}
...
value.itemModify(this.battle.gen<3?1.1:1.2);
return value;
}
...
moveCount['PhysicalSetup']++;
}else if(['nastyplot','manacharge','quiverdance','calmmind','geomancy'].includes(move.id)){
moveCount['SpecialSetup']++;
}
...
if(hasMove['willowisp']||hasMove['disperse']||hasMove['irondefense']||hasMove['cottonguard']){
physicalBulk*=1.6;
...
if(itemid==='powerribbon'&&moveCount['PhysicalAttack']>=2){
